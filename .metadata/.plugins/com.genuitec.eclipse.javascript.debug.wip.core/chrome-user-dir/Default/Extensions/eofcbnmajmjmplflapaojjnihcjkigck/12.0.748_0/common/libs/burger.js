	...
	Burger.Collection = {
		...
		addEvent : function(eventDetails){
			...
			if (numberOfEvents < Burger.Collection.events.BATCH_MAX_MESSAGES){
				...
				if(eventDetails.sendNow){
					Burger.Collection.sendAll(true);
				}
			}				
			else {
...
(function(_, AvastWRC) {
	...
	_.extend(Burger.Gpb,{
		Blob : PROTO.Message("Burger.Gpb.Blob",{	
			...
			PickedOffer : PROTO.Message("Burger.Gpb.Blob.PickedOffer",{
				...
				available_price_comparison: GPBD.int32(8, PROTO.optional)
			}),
			/**************************************************************************/
			/* ende Picked offer								  					  */
			/**************************************************************************/
	...
	_.extend(Burger.Build,{
		...
		buildAditionalInfoMessage: function(data){
			...
			aditionalInfo.event_type =  data.type ? Burger.Gpb.Blob.ClientEvent.AditionalInfo.EventType[data.type] : 0;
			aditionalInfo.ui_category = data.category ? Burger.Gpb.Blob.ClientEvent.AditionalInfo.UiCategory[data.category] : 0;
			aditionalInfo.ui_source = data.uiSource ? Burger.Gpb.Blob.ClientEvent.AditionalInfo.UiSource[data.uiSource] : 0;
		...
		pickedOfferMessage: function (offer, offerQuery, offerCategory){
			...
			if(offerCategory == "PRODUCT"){
				...
				pickedOffer.redirect["primary_image"] = offer["primaryImage"] || "";
				pickedOffer.redirect["secondary_image"] = offer["secondaryImage"] || "";
				pickedOffer.redirect["image_timeout"] = offer["imageTimeout"] || "";
				pickedOffer.redirect["category"] = offer["category"] || 0;
				pickedOffer.redirect["sub_category"] = offer["subCategory"] || 0;
				pickedOffer.redirect["provider_id"] = offer["providerId"] || "";
			}
				...
				pickedOffer.voucher["value"] = offer["value"] || 0;
